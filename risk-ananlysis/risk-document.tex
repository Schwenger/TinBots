...
\item[\texttt{standing still}]
    There are plenty of reasons for this behavior. \\
    First of all, the wheels could not turn, because of a primary motor fault or because they are blocked.\\
    Second of all, the setup is bad, e.g. the user placed the Tin Bot the wrong way around or on its side. \\
    Thirdly, there could be a software failure, i.e. an unhandled edge case.\\
    Additionally, the victim could have been pickup up through a wall, because the TODO
\item[\texttt{E-Puck failure}]
    An E-Puck fails if there is a failure regarding its power, or the controller board is faulty, the controller, meaning the software, if faulty, the user forgot to turn the E-Puck on, or there is a memory fault, such that invalid data is read leading to unpredictable behavior, such as standing still.
\item[\texttt{runs into walls}]
