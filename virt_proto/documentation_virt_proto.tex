...
In this case, we fall back to using the right hand rule while updating our knowledge about the world. Path Executor is a real state machine, but it is implemented as a Stateflow chart, as well as the Path Finder.
...
\begin{description}
\item[dgpve\_test] tests the physical model of differential drive isolated from any logic.
...
Use case \textbf{UC-C} corresponds to a nice-to-have feature and is not modeled. We did, however, not model more than one Tin Bot at a time, so the failure of one has no impact anyways.
