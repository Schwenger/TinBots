...
\testprot{Victim Direction Computation}{
    ...
    measurement are significantly better, as can be seen in Fig. \ref
    {fig:vddatagood}.
}

\subsection{Path Finder}
\subsubsection{Purpose}
Computes a path from the current position to the victim based on the internal 
map consisting of data gathered via proximity sensors and other Tin Bot's 
broadcasts.
\teststrat{Path Finder State Machine}{
    ...
    \BLACK Start respective MatLab test case from the virtual prototype using
